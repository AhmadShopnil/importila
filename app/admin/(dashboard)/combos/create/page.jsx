    ...
    <div className=" py-8 space-y-8">
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Create New Combo</h1>
        </div>
      </div>
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar">
                {selectedProducts?.length === 0 ? (
                  ...
                  selectedProducts.map((p, index) => (
                    <div key={p.productId} className="bg-muted/10 border border-border rounded-2xl p-4 relative group animate-in fade-in slide-in-from-bottom-2 duration-300">
                      <button
                        type="button"
                        onClick={() => setSelectedProducts(prev => prev.filter(item => item?._id !== p?._id))}
